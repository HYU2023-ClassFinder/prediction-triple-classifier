...
for n_estimators in [20]:
    ...
    x_labeled_test = x_labeled
    y_labeled_test = y_labeled
    ...
    for iteration in range(100):
        ...
        x_labeled = pd.concat([x_labeled, x_unlabeled.iloc[indices]])
        y_labeled = pd.concat([y_labeled, pd.Series(prediction[indices])])
    ...
    y_pred_test = regressor.predict(x_labeled_test[['score', 'triple_incidence', 'head_rate', 'tail_rate']])
    print(classification_report(y_labeled_test, y_pred_test))

    # Final prediction on the entire data and save to file
    prediction = regressor.predict(x_unlabeled[['score', 'triple_incidence', 'head_rate', 'tail_rate']])
    x = pd.concat([x_labeled, x_unlabeled])
    y = pd.concat([y_labeled, pd.Series(prediction)])
    x['predicted_class'] = y.values
    x.to_csv(f'nst_combined_data_class_x_n_est_{n_estimators}.csv', index=False)

    df['predicted_class'] = regressor.predict(df[['score', 'triple_incidence', 'head_rate', 'tail_rate']])
    # count the number of 1s
    print(df['predicted_class'].value_counts())
    df.to_csv(f'nst_combined_predicted_classification_n_est_{n_estimators}.csv', index=False)